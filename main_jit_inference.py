...
def get_args_parser():
    ...
    # SAR conditioning
    parser.add_argument('--mapper_depth', default=2, type=int, help='Number of transformer blocks in SAR mapper')
    parser.add_argument('--mapper_mlp_ratio', default=2.0, type=float, help='MLP ratio in SAR mapper')
    parser.add_argument('--mapper_attn_drop', default=0.0, type=float, help='Attention dropout in SAR mapper')
    parser.add_argument('--mapper_proj_drop', default=0.0, type=float, help='Projection dropout in SAR mapper')
    ...
    parser.add_argument('--dino_repo', default='./dinov3-main', type=str, help='Torch hub repo for DINOv3')
...
def main(args):
    ...
    if getattr(args, "distributed", False):
        ...
        #     b = model.prior_net.final_layer.linear.bias
    ...
    def print_prior_head(tag):
        if hasattr(model_without_ddp, "prior_net"):
            w = model_without_ddp.prior_net.final_layer.linear.weight
            print(f"[{tag}] prior final w_abs_max:", w.abs().max().item())
        else:
            print(f"[{tag}] no prior_net")

    print_prior_head("BEFORE resume load")
    ...
    if checkpoint_path and os.path.exists(checkpoint_path):
        ...
        print_prior_head("AFTER  resume load")

        if getattr(args, "prior_ckpt_path", None) and getattr(model_without_ddp, "subspace_enabled", False):
            prior_checkpoint = torch.load(args.prior_ckpt_path, map_location="cpu", weights_only=False)
            prior_state = prior_checkpoint["model"] if (
                        isinstance(prior_checkpoint, dict) and "model" in prior_checkpoint) else prior_checkpoint
            if isinstance(prior_state, dict):
                has_net_prefix = any(k.startswith("net.") for k in prior_state.keys())
                if has_net_prefix:
                    prior_state = {k.replace("net.", "", 1): v for k, v in prior_state.items()}

            model_without_ddp.prior_net.load_state_dict(prior_state, strict=True)
            model_without_ddp.prior_net.eval()
            for p in model_without_ddp.prior_net.parameters():
